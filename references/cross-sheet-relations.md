...
4. 多标段无法唯一拆分时，Python 将相关投标组降级合并为项目级组，清空 `lot_name/lot_code`，保留项目级中标结果并生成 `LOT_SCOPE_UNRESOLVED`；不得声称其他企业属于某一具体标段的未中标企业。
...
9. 目标 bidder roster 存在项目缺口、企业读取失败或声明数量不一致时，关系状态记为 `blocked`，不消费中标汇总、不生成项目/企业推荐；修复覆盖后再重建。

候选最多5个，优先展示无明确身份冲突及年度一致的候选。明确项目编号/年度冲突、显式资金类型或建设类型冲突、主工程与追加工程范围差异，以及缺少区分依据的同名同年度多来源，保存为 selection_blockers，不接受模型强行关联；缺失字段本身不当作冲突。唯一项目编号或实施主体可区分同名来源；仅有单元格地址不证明汇总对应哪一个来源块。候选不包含投标企业；中标企业不得参与项目生成或选择。没有可选择候选时保留独立复核，投标参与不丢失。
