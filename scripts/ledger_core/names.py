...
POLICY = {"version": 1, "company_name": "bidder_preferred", "method": "deterministic",
...
def candidate_scores(award: dict, records: list[dict], units: dict) -> list[dict]:
    # 同表平行报价列只标注一侧单位时按该单位比较，假设随组保存。
    bidder_unit = units.get("bidder_price") or units.get("award_price", "")
    award_unit = units.get("award_price") or units.get("bidder_price", "")
    award_amount = amount(award["price"], award_unit)
    candidates = []
    for record in records:
        ...
        for occurrence in record["occurrences"]:
            ...
            price = award_amount is not None and award_amount == amount(values.get("bidder_price", ""), bidder_unit)
            evidence.append((person, price))
        candidates.append({"record_id": record["id"], "name": record["name"],
                           ...
                           "person_amount_equal": (True, True) in evidence if award["single_name"] else False})
...
def _row_mode(group: dict, cases: list[dict]) -> dict:
    ...
    for case in cases:
        ...
        price_unique = own["amount_equal"] and sum(c["amount_equal"] for c in case["candidates"]) == 1
        supported = bool(case["exact"]) or own["person_amount_equal"] or (price_unique and own["core_similarity"] >= POLICY["min_core_similarity"])
...
def match_group(group: dict) -> tuple[set[str], list[dict]]:
    ...
    for entries in awards.values():
        ...
        cases.append({"award": award, "entries": entries, "candidates": candidates,
                      ...
                      "joint": [c for c in candidates if c["person_amount_equal"]]})
    ...
    for case in cases:
        ...
        if exact:
            ...
            if (best["name_similarity"] >= POLICY["min_full_similarity"] and
                    ...
                    best["score"] >= POLICY["min_score"] and margin >= POLICY["min_margin"]):
                selected, basis = best, "unique_name_similarity"
            elif (best["prefix_omitted"] and best["name_similarity"] >= POLICY["min_abbreviation_full"] and
                  margin >= POLICY["min_abbreviation_margin"]):
                selected, basis = best, "unique_abbreviated_name"
        case.update(selected=selected, basis=basis, conflict=conflict)
