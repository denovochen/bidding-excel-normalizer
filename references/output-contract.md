...
非精确名称生成一个人工确认任务。安全的单企业同行结构优先推荐同行投标企业；否则按组内名称接近程度推荐。名称评分并列时不设唯一推荐，展示有界候选和“不确定”，排序不能代替证据。推荐不改变中标状态。法人、报价、中标金额及单位继续保留在来源审计中，但不参与自动匹配或推荐排序，缺失时不产生匹配问题。
...
1.8.1 的返回结果新增 delivery：message 为程序生成的交付说明；review_reasons 按问题代码列出实际问题数和受影响队列记录数（不同原因可重叠），audit_warning_count 单独计数。模型不能在这些来源之外添加复核原因。项目内部选择标记 actor=model、basis=model_selection；中标选择标记 actor=user、actor_verified=false 并保留本地答案未获宿主验证的说明。旧审计字段仍可只读校验。
